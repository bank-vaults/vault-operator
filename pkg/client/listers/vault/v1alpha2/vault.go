...
type VaultLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.Vault, err error)
...
// List lists all Vaults in the indexer.
func (s *vaultLister) List(selector labels.Selector) (ret []*v1alpha2.Vault, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Vault))
	})
	return ret, err
}
...
type VaultNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.Vault, error)
...
// List lists all Vaults in the indexer for a given namespace.
func (s vaultNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.Vault, err error) {
...
// Get retrieves the Vault from the indexer for a given namespace and name.
func (s vaultNamespaceLister) Get(name string) (*v1alpha2.Vault, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("vault"), name)
	}
	return obj.(*v1alpha2.Vault), nil
}
